...
template < class V >
Matrix4<V> rotate_euler(const Vector3<V>& rotVec, EulerOrder::type rotOrder = EulerOrder::XYZ)
{
	switch(rotOrder) {
	case EulerOrder::XYZ:
...
template < class V >
Matrix4<V> rotate_axis_angle(const Vector3<V>& axis, V angle)
{
	V c = static_cast<V>(cos(deg_to_rad(angle)));
	V s = static_cast<V>(sin(deg_to_rad(angle)));
...
template < class V >
Matrix4<V> translate(const Vector3<V>& t)
{
	return Matrix4<V>(
...
template < class V >
Matrix3<V> translate(V x, V y)
{
	return Matrix3<V>(
		static_cast<V>(1), static_cast<V>(0), x,
		static_cast<V>(0), static_cast<V>(1), y,
		static_cast<V>(0), static_cast<V>(0), static_cast<V>(1));
}

template < class V >
Matrix3<V> translate(const Vector2<V>& t)
{
	return Matrix3<V>(
		static_cast<V>(1), static_cast<V>(0), t.x,
		static_cast<V>(0), static_cast<V>(1), t.y,
		static_cast<V>(0), static_cast<V>(0), static_cast<V>(1));
}


template < class V >
Vector3<V> get_translate(const Matrix4<V>& t)
{
	return Vector3<V>(t.getColumnVector(3)); 
}

template < class V >
Vector3<V> get_scale(const Matrix4<V>& t)
{
	return Vector3<V>(t.getColumnVector(0).length(), t.getColumnVector(1).length(), t.getColumnVector(2).length());
}
...
template < class V >
void split_transform(const Matrix4<V>& matrix, Quaternion<V>& rot, Vector3<V>& trans, Vector3<V>& sc)
{
	sc = get_scale(matrix);
	Matrix4<V> unscaledMat(matrix * scale(1 / sc.x, 1 / sc.y, 1 / sc.z));
...
template < class V >
Matrix4<V> scale(const Vector3<V>& t)
{
	return Matrix4<V>(
...
template < class V >
Matrix4<V> transform(const Matrix4<V>& rotMat, const Matrix4<V>& transMat, const Matrix4<V>& scaleMat)
{
	return (transMat * rotMat) * scaleMat;
}

template < class V >
Matrix4<V> transform(const Matrix4<V>& rotMat, const Matrix4<V>& transMat)
{
	return transMat * rotMat;
}

template < class V >
Matrix4<V> transform_reverse(const Matrix4<V>& rotMat, const Matrix4<V>& transMat, const Matrix4<V>& scaleMat)
{
	return transform(rotMat, transMat, scaleMat).inverse();
}

template < class V >
Matrix4<V> transform_reverse(const Matrix4<V>& rotMat, const Matrix4<V>& transMat)
{
	return transform(rotMat, transMat).inverse();
}

template < class V >
Matrix4<V> transform(const Vector3<V>& rotVec, const Vector3<V>& transVec, EulerOrder::type rotOrder = EulerOrder::XYZ)
{
	return transform(rotate_euler(rotVec, rotOrder), translate(transVec));
}

template < class V >
Matrix4<V> transform(const Vector3<V>& rotVec, const Vector3<V>& transVec, const Vector3<V>& scaleVec, EulerOrder::type rotOrder = EulerOrder::XYZ)
{
	return transform(rotate_euler(rotVec, rotOrder), translate(transVec), scale(scaleVec));
}

template < class V >
math::Matrix4<V> transform(const Quaternion<V>& rot, const Vector3<V>& trans, const Vector3<V>& sc)
{
	return transform(rot.to_matrix(), translate(trans), scale(sc));
}

template < class V >
Matrix4<V> transform_reverse(const Vector3<V>& rotVec, const Vector3<V>& transVec, EulerOrder::type rotOrder = EulerOrder::XYZ)
{
	return rotate_euler(rotVec * -1, EulerOrder::reverse_order(rotOrder))*translation(transVec * -1);
}

template < class V >
Matrix4<V> transform_reverse(const Vector3<V>& rotVec, const Vector3<V>& transVec, const Vector3<V>& scaleVec, EulerOrder::type rotOrder = EulerOrder::XYZ)
{
	return (scale(Vector3<V>(1 / scaleVec.x, 1 / scaleVec.y, 1 / scaleVec.z))*
		rotate_euler(rotVec * -1, EulerOrder::reverse_order(rotOrder)))*translation(transVec * -1);
}

template < class V >
Matrix4<V> look_at(const Vector3<V>& eyePosition3D, const Vector3<V>& center3D, Vector3<V> upVector3D )
{
	//------------------
	Vector3<V> forward = (center3D - eyePosition3D).normal();
	if(forward == upVector3D)
	{
		upVector3D = Vector3<V>(forward.z, forward.y, forward.x).normal();
	}
	//------------------
	//Side = forward x up
	Vector3<V> side = upVector3D.crossp(forward).normal();
	//------------------
	//Recompute up as: up = side x forward
	Vector3<V> up = forward.crossp(side).normal();
	//------------------
	Matrix4<V> matrix2;
	matrix2.setColumnVector(0, Vector4<V>(side, static_cast<V>(0)));
	matrix2.setColumnVector(1, Vector4<V>(up, static_cast<V>(0)));
	matrix2.setColumnVector(2, Vector4<V>(forward, static_cast<V>(0)));
	matrix2.setColumnVector(3, Vector4<V>(eyePosition3D, static_cast<V>(1)));
	return matrix2;
	//matrix2[0] = side[0];
	//matrix2[4] = side[1];
	//matrix2[8] = side[2];
