...
template < class FTy_ >
void Atmosphere<FTy_>::set_scale_heights(float_type scaleHeightRayleigh, float_type scaleHeightMie)
{
	_scaleHeightRayleigh = rayleighScaleHeight;
	_scaleHeightMie = mieScaleHeight;
}
...
void Atmosphere<FTy_>::set_k(float_type n, float_type Ns)
{
	...
	_beta = (float_type(4) * float_type(M_PI) * _K) / ::pow(LightWavelengths, 4);
}
...
template < class FTy_ >
typename Atmosphere<FTy_>::vec3_type Atmosphere<FTy_>::get_beta(float_type n, float_type Ns)
{
	return (float_type(4) * float_type(M_PI) * calculate_k(n, Ns)) / ::pow(LightWavelengths, 4);
}
...
template < class FTy_ >
void Atmosphere<FTy_>::calculate_optical_depth_tables(int samples, int angleSamples, int heightSamples)
{
	...
	calculate_optical_depth_table(samples, angleSamples, heightSamples, _rayleighScaleHeight, _rayleighOpticalDepthTable);
	calculate_optical_depth_table(samples, angleSamples, heightSamples, _mieScaleHeight, _mieOpticalDepthTable);
}

template < class FTy_ >
const std::vector<typename Atmosphere<FTy_>::float_type> Atmosphere<FTy_>::get_optical_depth_rayleigh_table() const 
{
	return _rayleighOpticalDepthTable;
}
template < class FTy_ >
const std::vector<typename Atmosphere<FTy_>::float_type> Atmosphere<FTy_>::get_optical_depth_mie_table() const 
{
	return _mieOpticalDepthTable;
}

template < class FTy_ >
void Atmosphere<FTy_>::calculate_optical_depth_table(int samples, int angleSamples, int heightSamples, float_type scaleHeight, std::vector<float_type>& results) const
{
	...
	for(int aIdx = 0, rIdx = 0; aIdx < angleSamples; ++aIdx, angle += _angleFrac)
	{
		...
		{
			results[rIdx] = calculate_optical_depth_height_angle(height, angle, _innerRadius, _outerRadius, scaleHeight, samples);
		}
	}
}

template < class FTy_ >
void Atmosphere<FTy_>::calculate_surface_lighting_table(float_type mieu, 
	...
	for(int aIdx = 0, rIdx = 0; aIdx < angleSamples; ++aIdx, angle += angleFrac)
	{
		...
		for(int hIdx = 0; hIdx < heightSamples; ++hIdx, height += heightFrac, ++rIdx)
		{
			...
			for(size_t sIdx = 0; sIdx < _quarterSphereSamples.size(); ++sIdx)
			{
				...
				rayleigh += irradiance_at_view_Iv(sunDir, eyePos, samplePt, float_type(0), _scaleHeightRayleigh, PaPbSteps, PPaSteps, PPcSteps);
				mie += irradiance_at_view_Iv(sunDir, eyePos, samplePt, mieg, _scaleHeightMie, PaPbSteps, PPaSteps, PPcSteps);
			}
...
template < class FTy_ >
typename Atmosphere<FTy_>::spectrum_type Atmosphere<FTy_>::irradiance_at_view_Iv(
	const vec3_type& sunDir, const vec3_type& eye, const vec3_type& eyeDir, 
	float_type g, float_type scaleHeight, int PaPbSteps, int PPaSteps, int PPcSteps) const
{
		...
		(planetISect.t1 <= 0 && planetISect.t2 <= 0))
	{
		...
		return calculate_irradiance_at_view_Iv(_beta, sunDir, startPos, atmosphereISect.p2, _innerRadius, _outerRadius, scaleHeight, _K, g, PaPbSteps, PPaSteps, PPcSteps);
	}
	...
	return calculate_irradiance_at_view_Iv(_beta, sunDir, startPos, planetISect.p1, _innerRadius, _outerRadius, scaleHeight, _K, g, PaPbSteps, PPaSteps, PPcSteps);
}
...
template < class FTy_ >
typename Atmosphere<FTy_>::spectrum_type Atmosphere<FTy_>::calculate_irradiance_at_view_Iv(const spectrum_type& beta, const vec3_type& sunDir, vec3_type Pa, vec3_type Pb, float_type innerRadius, float_type outerRadius, float_type scaleHeight, float_type K, float_type g, int PaPbSteps, int PPaSteps, int PPcSteps) const
{
	float_type theta = sunDir.angle((Pa - Pb).normal());
	spectrum_type KFrThetaOverLambda4 = K * get_scattering_phase_Fr(theta, g) / ::pow(LightWavelengths, 4);
