...
template < class V > struct Matrix4;
...
struct Matrix3
{
	...
	template < class V >
	Matrix3(const Vector3<V>& column1, const Vector3<V>& column2, const Vector3<V>& column3)
	...
	template < class V >
	explicit Matrix3(const Matrix3<V>& m)
		: m11(m.m11), m12(m.m12), m13(m.m13),
		m21(m.m21), m22(m.m22), m23(m.m23),
		m31(m.m31), m32(m.m32), m33(m.m33)
	{
	...
	template < class V >
	explicit Matrix3(const Matrix4<V>& m)
		: m11(m.m11), m12(m.m12), m13(m.m13),
		m21(m.m21), m22(m.m22), m23(m.m23),
		m31(m.m31), m32(m.m32), m33(m.m33)
	{
...
//  	}
	...
	template < class V >
	void setColumnVector(index_type col, const Vector3<V>& vec)
	{
		for (index_type i = 0; i < Size; i++)
			(*this)(i, col) = vec[i];
	}
...
// 	}
	...
	template < class V >
	Vector3<V> operator*(const Vector3<V>& u) 
	{
		return Vector3<V>
	...
	template < class V >
	this_type operator*(const Matrix3<V>& u)
	{
		return Matrix3<V>(
...
template < class V > 
const typename Matrix3<V>::this_type Matrix3<V>::Identity = 
	Matrix3<V,H>(1, 0, 0, 0, 1, 0, 0, 0, 1);

template < class V > 
const typename Matrix3<V>::index_type Matrix3<V>::Size = 3;
...
bool operator==(const LIB_NAMESPACE::math::Matrix3<U>& u, const LIB_NAMESPACE::math::Matrix3<V>& v) { 
...
template < class V >
std::ostream& operator<<(std::ostream& str, const LIB_NAMESPACE::math::Matrix3<V>& u)
{
