...
private:
	...
	struct ThreadData
	{
		ThreadData(const ThreadData& from)
			...
			scale2(from.scale2), 
			scale3(from.scale3), 
			center2(from.center2),
			center3(from.center3),
		...
		value_type scale3, center3;
		value_type scale2, center2;
		int octavesi;
...
public:
	...
	value_type operator()(const Vector2Type& vec) const;
	...
	void eval(SSEVectorType& results, const SIMDSoAVector2Type& v, 
		value_type vscale = 1) const;
