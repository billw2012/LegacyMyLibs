...
math::Vector3<V> pow(const math::Vector3<V>& _X, V2 _Y)
{
	return math::Vector3<V>(::pow(_X.x, _Y), ::pow(_X.y, _Y), ::pow(_X.z, _Y));
}

template < class V >
math::Vector3<V> exp(const math::Vector3<V>& _X)
{
	return math::Vector3<V>(::exp(_X.x), ::exp(_X.y), ::exp(_X.z));
}
...
struct Atmosphere 
{
	...
	// scale height is the thickness of the atmosphere if its density
	// were uniform (7994m for Earth's atmosphere for air, 1200m for aerosols)
	void set_scale_heights(float_type rayleighScaleHeight, float_type mieScaleHeight);

	// n is the index of refraction of the air
	// Ns is the molecular number density of the standard atmosphere
	// K is a resultant constant for the standard atmosphere (molecular density at sea level).
	...
	// u is determined by atmospheric conditions, varying from 0.7 to 0.85.
	// g is the resultant the asymmetry factor for the scattering function.
	static float_type get_g(float_type u);

	// optical depth at a single point, integrated along a path to get full optical depth
	static vec3_type get_beta(float_type n, float_type Ns);

	void calculate_optical_depth_tables(int samples, int angleSamples, int heightSamples);

	const std::vector<float_type> get_optical_depth_rayleigh_table() const ;
	const std::vector<float_type> get_optical_depth_mie_table() const ;

	void calculate_optical_depth_table(int samples, int angleSamples, int heightSamples, 
		float_type scaleHeight, std::vector<float_type>& results) const;

 	void calculate_surface_lighting_table(float_type mieu, 
	...
	spectrum_type irradiance_at_view_Iv(const vec3_type& sunDir, const vec3_type& eye, 
		const vec3_type& eyeDir, float_type g, float_type scaleHeight, int PaPbSteps, int PPaSteps, int PPcSteps) const;
...
private:
	...
	spectrum_type calculate_irradiance_at_view_Iv(const spectrum_type& beta, const vec3_type& sunDir, vec3_type Pa, vec3_type Pb, float_type innerRadius, float_type outerRadius, float_type scaleHeight, float_type K, float_type g, int PaPbSteps, int PPaSteps, int PPcSteps) const;
...
private:
	...
	float_type _innerRadius, _outerRadius, _K, _scaleHeightRayleigh, _scaleHeightMie;
	...
	std::vector<float_type> _rayleighOpticalDepthTable, _mieOpticalDepthTable;
