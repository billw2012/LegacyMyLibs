...
template < class V > struct Ray;
template < class V > struct Plane;
...
template < class V, class VP > 
IntersectionPair< V >
	intersects(const BoundingSphere<V>& bsph, const Plane<VP>& plane)
{
...
IntersectionPair< V >
	intersects(const BoundingSphere<V>& bsph, const AABB<V2>& aabb)
{
...
template < class V > 
IntersectionPair< V >
intersects( const BoundingSphere<V>& sphere, const Frustum<V>& frustum )
{
	//Vector3<V> op = sphere.center() - frustum.eyePos();   
	...
	{
		if(frustum.get_plane(idx).distance(sphere.center()) < -sphere.radius())
			return IntersectionPair<V>();
	}
	return IntersectionPair<V>(true);
}
