...
{
	emit process_output(QString::fromLocal8Bit(buff, static_cast<int>(count)));
}
...
void ProcessQueue::run_command( const ProcessCommand& cmd )
{
	...
	run_redirected_process(cmd.commandLine.toLocal8Bit().data(), boost::bind(&ProcessQueue::process_output_callback, this, boost::ref(cmd), _1, _2));
}
