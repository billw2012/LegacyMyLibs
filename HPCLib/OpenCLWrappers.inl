...
template < class ValTy_ >
cl_event OpenCLBuffer<ValTy_>::enqueue_read(const CLDevice& device_, bool blocking, size_t elementOffset, size_t elementCount)
{
	cl_event event = NULL;
...
template < class ValTy_ >
cl_event OpenCLBuffer<ValTy_>::enqueue_write(const CLDevice& device_, bool blocking, size_t elementOffset, size_t elementCount)
{
	cl_event event = NULL;
...
// #if CL_VERSION_1_2
template < class ValTy_ > 
bool CLProgram::bind_parameter(const std::string& kernelFnName, const std::string& paramName, const ValTy_& val) const
{
	KernalMap::const_iterator fItr = _kernals.find(kernelFnName);
	if(fItr == _kernals.end())
		return false;
	const CLKernel& kernel = fItr->second;
	CLKernel::ArgumentMap::const_iterator argItr = kernel.arguments.find(paramName);
	if(argItr == kernel.arguments.end())
		return false;
	_lastError = ::clSetKernelArg(fItr->second.kernel, argItr->second, sizeof(ValTy_), &val);
	return _lastError == CL_SUCCESS;
}

template < class ValTy_ > 
bool CLProgram::bind_parameter(const std::string& kernelFnName, const std::string& paramName, const OpenCLBuffer<ValTy_>& buffer) const
{
	return bind_parameter(kernelFnName, paramName, buffer.get_handle());
}
// #endif

template < class ValTy_ > 
bool CLProgram::bind_parameter(const std::string& kernelFnName, cl_uint paramIdx, const ValTy_& val) const
{
	KernalMap::const_iterator fItr = _kernals.find(kernelFnName);
	if(fItr == _kernals.end())
		return false;
...
template < class ValTy_ > 
bool CLProgram::bind_parameter(const std::string& kernelFnName, cl_uint paramIdx, const OpenCLBuffer<ValTy_>& buffer) const
{
